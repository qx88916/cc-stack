...
const LOCAL_API = API_BASE_URL; // Use centralized config
...
interface AuthContextValue {
  ...
  loginWithPhone: (phone: string, password: string) => Promise<{ error?: string; user?: AuthUser }>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loginWithPhone = useCallback(
    async (phone: string, password: string) => {
      try {
        const res = await fetch(`${apiBaseUrl}/auth/login-phone`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <AuthContext.Provider
      value={{ session, isLoading, apiBaseUrl, setApiBaseUrl, login, loginWithPhone, signup, logout, refreshSession }}
    >
      {children}
    </AuthContext.Provider>
  );
