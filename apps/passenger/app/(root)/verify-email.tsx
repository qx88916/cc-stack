...
import { COLORS, SHADOW_SM, SHADOW_MD } from '@/constants/theme';
...
export default function VerifyEmail() {
  ...
  const handleSendCode = async () => {
    ...
    setLoading(true);
    try {
      await fetchAPI('/auth/send-email-otp', {
        method: 'POST',
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      await fetchAPI('/user/verify-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
    ...
    <SafeAreaView className="flex-1 bg-general-500">
      <View className="flex-1">
        ...
        <View className="flex-row items-center px-5 pt-3 pb-4">
          ...
          <View>
            <Text className="text-xl font-JakartaBold text-neutral-900">Verify Your Email</Text>
        ...
        <View className="flex-1 px-5">
          ...
          <View className="bg-white p-4 rounded-2xl mb-5" style={SHADOW_SM}>
            <Text className="text-neutral-400 text-xs font-JakartaMedium mb-1">Email Address</Text>
            <Text className="font-JakartaBold text-base text-neutral-800">{session?.user?.email}</Text>
          </View>
            ...
            <View>
              <View className="bg-secondary-100 border border-secondary-200 p-4 rounded-2xl mb-5">
                <View className="flex-row items-center mb-1">
                  <Image source={icons.email} className="w-4 h-4 mr-1.5" tintColor={COLORS.secondary} resizeMode="contain" />
                ...
                <Text className="text-secondary-700 text-xs font-JakartaMedium">
                  Check your inbox for a 6-digit verification code. The code will expire in 10 minutes.
                </Text>
              </View>
          ...
          <View className="mt-auto pb-8 pt-6">
            ...
            {[
              ...
              'Ride receipts and important notifications',
